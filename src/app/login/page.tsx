...
function LoginContent() {
  const { user, loginProfessional, loginNormal, logout, authError } = useAuth();
  ...
  const onProfessionalLogin = async () => {
    try {
      setLoading(true);
      setError("");
      await loginProfessional();
      router.push(`/auth/status?type=Professional&next=${encodeURIComponent(next)}`);
    } catch (nextError) {
      setError(nextError instanceof Error ? nextError.message : "Professional login failed.");
    } finally {
      setLoading(false);
    }
  };

  const onNormalLogin = async (event: React.FormEvent) => {
    event.preventDefault();
    try {
      setLoading(true);
      setError("");
      await loginNormal(credentials);
      router.push(`/auth/status?type=Normal&next=${encodeURIComponent(next)}`);
    } catch (nextError) {
      setError(nextError instanceof Error ? nextError.message : "Normal login failed.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="grid gap-12 lg:grid-cols-2">
        ...
        <div className="relative">
          <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-200/60">
            ...
            <div className="p-8">
              <form onSubmit={onNormalLogin} className="space-y-4">
                {mode === "signup" && (
                  <div className="relative">
                ...
                >
                  {loading ? (
                    ...
                    <span>{mode === "login" ? "Normal Login" : "Normal Sign Up"}</span>
                  )}
                </button>
              </form>
